...
uint8_t fat32_init() {
    ...
    FAT32.sectorsPerCluster = data[0];
...
uint32_t directory_find(char *folderName){
    ...
    return (((startCluster - 2) * FAT32.sectorsPerCluster) + FAT32.rootStart);
...
void file_finds (uint32_t directorySector, struct file *allFiles){
    ...
    for (j = 0; j < 2; j++){
        ...
        for (entry = 0; entry < 12; entry++){
            ...
            newFile.sector = ((newFile.cluster - 2) * FAT32.sectorsPerCluster) 
                    + FAT32.rootStart;
            //get file size
            offset += 0x02;
...
uint8_t file_read(struct file * currentFile, uint8_t * buffer){
    ...
    if (currentFile->sectorInCluster == 64){
        ...
        currentFile->sector = ((currentFile->cluster - 2) * FAT32.sectorsPerCluster) 
            + FAT32.rootStart;
