...
void init_songs() {
    ...
    if (num_songs >= 10) {
        num_songs_1 = 10;
        num_songs_2 = num_songs - 10;
    }
    else {
        num_songs_1 = num_songs;
    }

    i=0;
    int col;
    int row;
    //creates buttons for each song on the first page
    for (col=0; col<2; col++){
        for (row=0; row<5; row++){
            if (i > num_songs_1) break;
            Button song = {.x1=3+col*110, .y1=30+row*40, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT, .title='temp'};
            strcpy(song.title, music_files[i].fileName);
            song_select_buttons_1[i] = song;
            i++;
        }
        if (i > num_songs_1) break;
    }
    int n = 0;
    //creates song buttons for second page
    if (num_songs_2 > 0) {
        for (col=0; col<2; col++){
            for (row=0; row<5; row++){
                if (n > num_songs_2) break;
                Button song = {.x1=3+col*110, .y1=30+row*40, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT2, .title='temp'};
                strcpy(song.title, music_files[i].fileName);
                song_select_buttons_2[n] = song;
                n++;
                i++;
            }
            if (n > num_songs_2) break;
    ...
    //initializes the song navigation buttons on each page
    Button next_page_1 = {.x1=220, .y1=30, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT2, .title="Page 2"};
    Button confirm_1 = {.x1=220, .y1=130, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SCAN_FINGER, .title="Done"};
    Button demo_1 = {.x1=220, .y1=85, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT, .title="Demo"};
    song_nav_buttons_1[0]=next_page_1;
    song_nav_buttons_1[1]=demo_1;
    song_nav_buttons_1[2]=confirm_1;

    Button next_page_2 = {.x1=220, .y1=30, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT, .title="Page 1"};
    Button confirm_2 = {.x1=220, .y1=130, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SCAN_FINGER, .title="Done"};
    Button demo_2 = {.x1=220, .y1=85, .x2=80, .y2=30, .color=WHITE, .next_state=ENROLL_SONG_SELECT2, .title="Demo"};
    song_nav_buttons_2[0]=next_page_2;
    song_nav_buttons_2[1]=demo_2;
    song_nav_buttons_2[2]=confirm_2;

    //set unknown config
    allEnrolls[201] = music_files[0];
}
...
Open();//Opens fingerprint scanner
...
delay_ms(10);
  ...
  while (1){
      ...
      PT_SCHEDULE(protothread_WriteUART(&pt_WriteUART));
